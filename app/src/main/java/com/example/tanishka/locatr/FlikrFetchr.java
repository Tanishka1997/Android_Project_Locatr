...
public class FlikrFetchr {
   ...
   List<PicItem> items;
    public static final String KEY="27f183c50460e9bcc2dd69cc4c28bf96";
    ...
    private static final Uri ENDPOINT=Uri.parse( "https://api.flickr.com/services/rest/").buildUpon().appendQueryParameter("api_key",KEY)
            .appendQueryParameter("format","json").appendQueryParameter("nojsoncallback","1").appendQueryParameter("extras","url_s,geo").build();
...
public  List<PicItem> searchPhotos(Location location){
 ...
 public void parseItems(JSONObject jsonBody,List<PicItem> items) throws IOException,JSONException{
     ...
     for(int i=0;i<array.length();i++){
         ...
         item.setmLat(array.getJSONObject(i).getDouble("latitude"));
         item.setmLon(array.getJSONObject(i).getDouble("longitude"));
         items.add(item);
     }
 }
}
